...
module internal HashImplementation = 
    ...
    module SetLinked =
        ...
        let rec applyDeltaNoState
            (apply : ('K -> bool -> 'D -> struct(bool * voption<'DOut>)))
            (delta : MapLinked<'K, 'D>) =
            ...
            let result =
                if isNull delta then
                    ...
                    let struct(exists, op) = (apply (delta.Key) (false) (delta.Value))
        ...
        let rec applyDelta
            ...
            let result =
                if isNull state then
                    ...
                    let struct(exists, op) = (apply (delta.Key) (wasExisting) (delta.Value))
    ...
    module MapLinked =
        ...
        let rec toListMap (mapping : ('K -> 'V -> 'T)) (acc : list<'T>) (n : MapLinked<'K, 'V>) =
            if isNull n then acc
            else (mapping (n.Key) (n.Value)) :: toListMap mapping acc n.MapNext
        ...
        let rec fold (folder : ('S -> 'K -> 'V -> 'S)) (state : 'S) (node : MapLinked<'K, 'V>) =
            if isNull node then
                state
            else
                let s1 = (folder (state) (node.Key) (node.Value))
                fold folder s1 node.MapNext
                       
        let rec exists (predicate : ('K -> 'V -> bool)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                false
            else
        ...
        let rec forall (predicate : ('K -> 'V -> bool)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                true
            else
                (predicate (node.Key) (node.Value)) &&
        ...
        let rec map (mapping : ('K -> 'V -> 'T)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                null
            else
                MapLinked(node.Key, (mapping (node.Key) (node.Value)), map mapping node.MapNext)
                 
        let rec filter (predicate : ('K -> 'V -> bool)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                null
            elif (predicate (node.Key) (node.Value)) then
                MapLinked(node.Key, node.Value, filter predicate node.MapNext)
                
            else
                filter predicate node.MapNext

        let rec choose (mapping : ('K -> 'V -> option<'T>)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                null
            else
                match (mapping (node.Key) (node.Value)) with
                | Some v ->
                    MapLinked(node.Key, v, choose mapping node.MapNext)
                | None ->
                    choose mapping node.MapNext

        let rec chooseV (mapping : ('K -> 'V -> voption<'T>)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                null
            else
                match (mapping (node.Key) (node.Value)) with
        ...
        let rec intersect 
            ...
            if isNull a || isNull b then null
                ...
                match ok with
                | ValueSome vb ->
                    let value = (resolve (a.Key) (a.Value) (vb))
        ...
        let rec choose2VLeft
            (mapping : ('K -> voption<'A> -> voption<'B> -> voption<'C>))
            (a : MapLinked<'K, 'A>)  =
            if isNull a then
                null
            else
                match (mapping (a.Key) (ValueSome a.Value) (ValueNone)) with
                | ValueSome c ->
        ...
        let rec choose2VRight
            ...
            if isNull b then
                ...
                match (mapping (b.Key) (ValueNone) (ValueSome b.Value)) with
                | ValueSome c ->
        ...
        let rec choose2V
            ...
            if isNull a then
                ...
                match res with
                | ValueSome vb ->
                    match (mapping (a.Key) (ValueSome a.Value) (ValueSome vb)) with
                    | ValueSome c ->
        ...
        let rec unionWithSelf (mapping : ('K -> 'V -> 'V -> 'V)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                null
            else
                MapLinked(node.Key, (mapping (node.Key) (node.Value) (node.Value)), unionWithSelf mapping node.MapNext)
        ...
        let rec unionWith
            ...
            if isNull a then b
                ...
                match res with
                | ValueSome aValue ->
                    let v = (resolve (b.Key) (aValue) (b.Value))
                    MapLinked(b.Key, v, unionWith cmp resolve a b.MapNext)
                | ValueNone -> 
                    MapLinked(b.Key, b.Value, unionWith cmp resolve a b.MapNext)
                
        let rec unionWithSelfV (mapping : ('K -> 'V -> 'V -> voption<'V>)) (node : MapLinked<'K, 'V>) =
            if isNull node then
                null
            else
                match (mapping (node.Key) (node.Value) (node.Value)) with
                | ValueSome v ->
        ...
        let rec unionWithV
            ...
            if isNull a then b
                ...
                match res with
                | ValueSome aValue ->
                    match (resolve (b.Key) (aValue) (b.Value)) with
                    | ValueSome v ->
        ...
        let rec computeDelta 
            (cmp : IEqualityComparer<'K>)
            (onlyLeft : ('K -> 'V -> voption<'OP>)) 
            (onlyRight : ('K -> 'V -> voption<'OP>)) 
            (both : ('K -> 'V -> 'V -> voption<'OP>)) 
            (a : MapLinked<'K, 'V>) (b : MapLinked<'K, 'V>) =
            ...
            if isNull a then 
                ...
                match res with
                | ValueSome bv ->
                    match (both (a.Key) (a.Value) (bv)) with
                    | ValueNone ->  
                ...
                | ValueNone ->
                    match (onlyLeft (a.Key) (a.Value)) with
                    | ValueNone ->  
                        computeDelta cmp onlyLeft onlyRight both a.MapNext b
                    | ValueSome v ->
                        MapLinked(a.Key, v, computeDelta cmp onlyLeft onlyRight both a.MapNext b)
        ...
        let rec applyDeltaNoState
            ...
            let result =
                if isNull delta then
                    ...
                    let struct(newValue, op) = (apply (delta.Key) (ValueNone) (delta.Value))
    ...
    module SetNode =
        ...
        let rec fold (folder : ('S -> 'K -> 'S)) (state : 'S) (node : SetNode<'K>) =
            if isNull node then
                state
            elif node.IsLeaf then
                let node = node :?> SetLeaf<'K>
    ...
    module MapNode =    
        ...
        let rec toListMap (mapping : ('K -> 'V -> 'T)) (acc : list<'T>) (node : SetNode<'K>) =  
            if isNull node then
                acc
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
                (mapping (node.Key) (node.Value)) :: MapLinked.toListMap mapping acc node.MapNext
            else
                let node = node :?> Inner<'K>
        ...
        let rec exists (predicate : ('K -> 'V -> bool)) (node : SetNode<'K>) =
            if isNull node then
                false
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
        ...
        let rec forall (predicate : ('K -> 'V -> bool)) (node : SetNode<'K>) =
            if isNull node then
                true
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
        ...
        let rec fold (folder : ('S -> 'K -> 'V -> 'S)) (state : 'S) (node : SetNode<'K>) =
            if isNull node then
                state
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
                let state = (folder (state) (node.Key) (node.Value))
        ...
        let rec iter (action : ('K -> 'V -> unit)) (node : SetNode<'K>) =
            if isNull node then
                ()
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
        ...
        let rec map (mapping : ('K -> 'V -> 'T)) (node : SetNode<'K>) =
            if isNull node then
                null
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
                MapLeaf(node.Hash, node.Key, (mapping (node.Key) (node.Value)), MapLinked.map mapping node.MapNext) :> SetNode<_>
            else
                let node = node :?> Inner<'K>
                Inner(node.Prefix, node.Mask, map mapping node.Left, map mapping node.Right) :> SetNode<_>

        let rec filter (predicate : ('K -> 'V -> bool)) (node : SetNode<'K>) =
            if isNull node then
                null
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
        ...
        let rec choose (mapping : ('K -> 'V -> option<'T>)) (node : SetNode<'K>) =
            if isNull node then
                null
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
        ...
        let rec chooseV (mapping : ('K -> 'V -> voption<'OP>)) (node : SetNode<'K>) =
            if isNull node then
                null
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
        ...
        let rec private choose2VRight
            ...
            if isNull na then 
                ...
                match (mapping (a.Key) (ValueNone) (ValueSome a.Value)) with
                | ValueSome c ->
        ...
        let rec unionWithSelf (mapping : ('K -> 'V -> 'V -> 'V)) (node : SetNode<'K>) =
            if isNull node then
                null
            elif node.IsLeaf then
                let node = node :?> MapLeaf<'K, 'V>
                MapLeaf(node.Hash, node.Key, (mapping (node.Key) (node.Value) (node.Value)), MapLinked.unionWithSelf mapping node.MapNext) :> SetNode<_>
            else
                let node = node :?> Inner<'K>
        ...
        let rec applyDeltaNoState
            ...
            let result =
                if isNull delta then
                    ...
                    let delta = delta :?> MapLeaf<'K, 'D>
                    let struct(exists, op) = (apply (delta.Key) (ValueNone) (delta.Value))
                    match op with
                    | ValueSome op ->
...
type HashMapEnumerator<'K, 'V, 'T> =
    ...
    val mutable internal Mapping : ('K -> 'V -> 'T)
    val mutable internal Value : 'T


    member x.MoveNext() =
        if not (isNull x.Next) then
            x.Value <- (x.Mapping (x.Next.Key) (x.Next.Value))
            ...
            x.Value <- (x.Mapping (node.Key) (node.Value))
            x.Next <- node.MapNext
    ...
    internal new(root : SetNode<'K>, mapping : ('K -> 'V -> 'T)) =
        {
            Root = root
            Mapping = mapping
            Head = root
...
type internal HashMapEnumerable<'K, 'V, 'T>(root : SetNode<'K>, mapping : ('K -> 'V -> 'T)) =
...
type HashSet<'K> internal(comparer : IEqualityComparer<'K>, root : SetNode<'K>) =
    ...
    static member ApplyDelta(a : HashSet<'K>, b : HashMap<'K, 'D>, apply : 'K -> bool -> 'D -> struct(bool * voption<'DOut>)) =
        let state = a.Root
        let struct(delta, state) = SetNode.applyDelta a.Comparer (apply) state b.Root
...
and [<Struct; DebuggerDisplay("Count = {Count}"); DebuggerTypeProxy(typedefof<HashMapProxy<_, _>>); CustomEquality; NoComparison; StructuredFormatDisplay("{AsString}"); CompiledName("FSharpHashMap`2")>] 
    ...
    static let tupleGetter = (fun k v -> (k,v))
    static let valueTupleGetter = (fun k v -> struct(k,v))
    static let keyGetter = (fun k _ -> k)
    static let valueGetter = (fun _ v -> v)
    static let kvpGetter = (fun k v -> KeyValuePair(k, v))
    ...
    static member ApplyDelta(a : HashMap<'K, 'V>, b : HashMap<'K, 'T>, apply : 'K -> voption<'V> -> 'T -> struct(voption<'V> * voption<'U>)) =
        let state = a.Root
        let struct(delta, state) = MapNode.applyDelta a.Comparer (apply) state b.Root
    ...
    member x.Choose2(other : HashMap<'K, 'T>, mapping : 'K -> option<'V> -> option<'T> -> option<'U>) =
        ...
        let realMapping = (fun k l r -> (mapping (k) (o l) (o r)) |> vo)
    ...
    member x.Map2V(other : HashMap<'K, 'T>, mapping : 'K -> voption<'V> -> voption<'T> -> 'U) =
        let mapping = mapping
        let realMapping = (fun k l r -> (mapping (k) (l) (r)) |> ValueSome)
        HashMap<'K, 'U>(comparer, MapNode.choose2V comparer realMapping root other.Root)
    ...
    member x.Map2(other : HashMap<'K, 'T>, mapping : 'K -> option<'V> -> option<'T> -> 'U) =
        ...
        let inline o v = match v with | ValueSome v -> Some v | ValueNone -> None
        let inline vo v = match v with | Some v -> ValueSome v | None -> ValueNone
        let realMapping = (fun k l r -> (mapping (k) (o l) (o r)) |> ValueSome)
        HashMap<'K, 'U>(comparer, MapNode.choose2V comparer realMapping root other.Root)
