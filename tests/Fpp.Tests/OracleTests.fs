...
let computationExpressionOracle =
    testList "oracle: computation expressions" [
        ...
        // F# prefers Bind2Return/Bind3Return over BindReturn composed with
        // MergeSources, and Bind2/Bind3 over Bind composed with them. Beyond
        // three sources there is no BindN, and the merge nests: the first two
        // stay put and the rest fold into a third.
        oracle "and! prefers Bind2Return, Bind3Return and Bind2" [
            "type M<'a> = M of 'a"
            "type A() ="
            "    member _.Bind (v : M<'a>, f : 'a -> M<'b>) : M<'b> ="
            "        printfn \"Bind\""
            ...
            "        printfn \"BindReturn\""
            "        match v with M x -> M (f x)"
            "    member _.Bind2 (a : M<'a>, b : M<'b>, f : 'a * 'b -> M<'c>) : M<'c> ="
            "        printfn \"Bind2\""
            "        match a, b with M p, M q -> f (p, q)"
            "    member _.Bind2Return (a : M<'a>, b : M<'b>, f : 'a * 'b -> 'c) : M<'c> ="
            "        printfn \"Bind2Return\""
            "        match a, b with M p, M q -> M (f (p, q))"
            "    member _.Bind3Return (a : M<'a>, b : M<'b>, c : M<'c>, f : 'a * 'b * 'c -> 'd) : M<'d> ="
            "        printfn \"Bind3Return\""
            "        match a, b, c with M p, M q, M r -> M (f (p, q, r))"
            "    member _.MergeSources (a : M<'a>, b : M<'b>) : M<'a * 'b> ="
            "        printfn \"MergeSources\""
            "        match a, b with M p, M q -> M (p, q)"
            "    member _.MergeSources3 (a : M<'a>, b : M<'b>, c : M<'c>) : M<'a * 'b * 'c> ="
            "        printfn \"MergeSources3\""
            "        match a, b, c with M p, M q, M r -> M (p, q, r)"
            "    member _.Return (v : 'a) : M<'a> ="
            "        printfn \"Return\""
            "        M v"
            "let a = A()"
            ...
            "        return p + q + r"
            "    }"
            "let o1 = print (match two with M v -> v)"
            "let o2 = print (match three with M v -> v)"
            "let o3 = print (match four with M v -> v)"
            "let o4 = print (match mixed with M v -> v)"
        ]
