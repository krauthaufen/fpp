...
let oracleTests =
    testList "oracle: F# vs F++" [
        ...
        oracle "Array: partition, grouping, concat and folds" [
            ...
            "    print (si (Array.singleton 5) + string (Array.isEmpty [||]))"
        ]
        oracle "Seq: the pieces Fable programs reach for" [
            "let xs = [ 3; 1; 4; 1; 5; 9; 2; 6 ] :> seq<int>"
            "let si (ys : seq<int>) = String.concat \",\" (List.map (fun v -> string v) (Seq.toList ys))"
            "let go ="
            "    print (si (Seq.map2 (fun a b -> a + b) (List.toSeq [1;2;3]) (List.toSeq [10;20;30])))"
            "    print (si (Seq.concat (List.toSeq [ List.toSeq [1;2]; List.toSeq [3] ])))"
            "    print (string (Seq.item 2 xs))"
            "    print (match Seq.tryItem 99 xs with Some v -> string v | None -> \"-\")"
            "    print (string (Seq.findIndex (fun x -> x > 4) xs))"
            "    print (match Seq.tryFindIndex (fun x -> x > 99) xs with Some v -> string v | None -> \"-\")"
            "    print (string (Seq.maxBy (fun x -> -x) xs) + \" \" + string (Seq.minBy (fun x -> -x) xs))"
            "    print (si (Seq.takeWhile (fun x -> x < 5) xs))"
            "    print (si (Seq.skipWhile (fun x -> x < 5) xs))"
            "    print (String.concat \",\" (List.map (fun (k, c) -> string k + \":\" + string c) (Seq.toList (Seq.countBy (fun x -> x % 3) xs))))"
            "    print (string (Seq.exactlyOne (List.toSeq [ 7 ])))"
            "    print (string (Seq.foldBack (fun x s -> x - s) (List.toSeq [1;2;3]) 0))"
            "    print (string (Seq.average (List.toSeq [1.0; 2.0; 6.0])))"
            "    print (string (Seq.isEmpty Seq.empty))"
            "    print (String.concat \",\" (List.map (fun (a, b) -> string a + b) (Seq.toList (Seq.zip (List.toSeq [1;2]) (List.toSeq [\"a\";\"b\"])))))"
        ]
        oracle "Set and Map: the remaining module surface" [
            "let s1 = Set.ofList [ 1; 2; 3; 4 ]"
            "let s2 = Set.ofList [ 3; 4; 5 ]"
            "let si (ys : int list) = String.concat \",\" (List.map (fun v -> string v) ys)"
            "let m = Map.ofList [ (1, \"a\"); (2, \"b\"); (3, \"c\") ]"
            "let go ="
            "    print (string (Set.foldBack (fun x acc -> x - acc) s1 0))"
            "    let inS, outS = Set.partition (fun x -> x % 2 = 0) s1"
            "    print (si (Set.toList inS) + \" | \" + si (Set.toList outS))"
            "    print (si (Set.toList (Set.unionMany [ s1; s2 ])))"
            "    print (si (Set.toList (Set.intersectMany [ s1; s2 ])))"
            "    print (string (Set.isProperSubset (Set.ofList [1;2]) s1))"
            "    print (string (Set.isProperSubset s1 s1))"
            "    print (string (Set.isProperSuperset s1 (Set.ofList [1;2])))"
            "    print (match Map.tryFindKey (fun k v -> v = \"b\") m with Some k -> string k | None -> \"-\")"
            "    print (string (Map.findKey (fun k v -> v = \"c\") m))"
            "    print (match Map.tryPick (fun k v -> if k = 2 then Some (v + \"!\") else None) m with Some r -> r | None -> \"-\")"
            "    print (Map.pick (fun k v -> if k = 3 then Some v else None) m)"
            "    let ev, od = Map.partition (fun k v -> k % 2 = 0) m"
        ...
        oracle "math: floor, ceil, banker's round, pown, Double/Int32 statics" [
            "let go ="
            "    print (string (floor 2.7) + \" \" + string (floor (0.0 - 2.7)))"
            "    print (string (ceil 2.1) + \" \" + string (ceil (0.0 - 2.1)))"
            "    print (string (round 2.5) + \" \" + string (round 3.5) + \" \" + string (round 2.4))"
            "    print (string (round (0.0 - 2.5)))"
            "    print (string (log10 1000.0))"
            "    print (string (pown 2.0 10) + \" \" + string (pown 2.0 (0 - 2)))"
        ...
        oracle "List: the zip3/transpose/scanBack family" [
            "let si (ys : int list) = String.concat \",\" (List.map (fun v -> string v) ys)"
            "let go ="
            "    print (String.concat \",\" (List.map (fun (a, b, c) -> string a + string b + string c) (List.zip3 [1;2] [3;4] [5;6])))"
            "    let ra, rb, rc = List.unzip3 [ (1, 2, 3); (4, 5, 6) ]"
            "    print (si ra + \" | \" + si rb + \" | \" + si rc)"
            "    print (si (List.map3 (fun a b c -> a + b + c) [1;2] [10;20] [100;200]))"
            "    print (si (List.mapi2 (fun i a b -> i + a + b) [1;2] [10;20]))"
            "    print (string (List.foldBack2 (fun a b s -> s + a * b) [1;2] [3;4] 0))"
            "    print (si (List.scanBack (fun x s -> x + s) [1;2;3] 0))"
            "    print (String.concat \" / \" (List.map si (List.transpose [ [1;2;3]; [4;5;6] ])))"
            "    print (si (List.permute (fun i -> 2 - i) [1;2;3]))"
            "    print (si (List.insertManyAt 1 [8;9] [1;2]))"
            "    print (si (List.removeManyAt 1 2 [1;2;3;4]))"
        ]
        oracle "Array: the zip3/transpose/scanBack family and sortInPlace" [
            "let si (ys : int[]) = String.concat \",\" (List.map (fun v -> string v) (Array.toList ys))"
            "let go ="
            "    print (String.concat \",\" (List.map (fun (a, b, c) -> string a + string b + string c) (Array.toList (Array.zip3 [|1;2|] [|3;4|] [|5;6|]))))"
            "    let ra, rb, rc = Array.unzip3 [| (1, 2, 3); (4, 5, 6) |]"
            "    print (si ra + \" | \" + si rb + \" | \" + si rc)"
            "    print (si (Array.map3 (fun a b c -> a + b + c) [|1;2|] [|10;20|] [|100;200|]))"
            "    print (si (Array.mapi2 (fun i a b -> i + a + b) [|1;2|] [|10;20|]))"
            "    print (string (Array.foldBack2 (fun a b s -> s + a * b) [|1;2|] [|3;4|] 0))"
            "    print (si (Array.scanBack (fun x s -> x + s) [|1;2;3|] 0))"
            "    let ys, st = Array.mapFoldBack (fun x s -> x * 2, s + x) [|1;2;3|] 0"
            "    print (si ys + \" | \" + string st)"
            "    print (String.concat \" / \" (List.map si (Array.toList (Array.transpose [| [|1;2;3|]; [|4;5;6|] |]))))"
            "    print (si (Array.permute (fun i -> 2 - i) [|1;2;3|]))"
            "    print (string (Array.findIndexBack (fun x -> x < 3) [|1;2;5|]))"
            "    print (si (Array.insertManyAt 1 [|8;9|] [|1;2|]))"
            "    print (si (Array.removeManyAt 1 2 [|1;2;3;4|]))"
            "    let m = [| 3; 1; 2 |]"
            "    let u = Array.sortInPlace m"
            "    print (si m)"
        ]
        oracle "Seq: fold2, forall2, zip3, unfold; Map extremes" [
            "let go ="
            "    print (string (Seq.fold2 (fun s a b -> s + a * b) 0 (List.toSeq [1;2]) (List.toSeq [3;4])))"
            "    print (string (Seq.forall2 (fun a b -> a < b) (List.toSeq [1;2]) (List.toSeq [3;4])))"
            "    print (String.concat \",\" (List.map (fun (a, b, c) -> string a + string b + string c) (Seq.toList (Seq.zip3 (List.toSeq [1;2]) (List.toSeq [3;4]) (List.toSeq [5;6])))))"
            "    print (String.concat \",\" (List.map (fun v -> string v) (Seq.toList (Seq.unfold (fun i -> if i < 4 then Some (i, i + 1) else None) 0))))"
            "    let m = Map.ofList [ (2, \"b\"); (1, \"a\"); (3, \"c\") ]"
            "    let kmin, vmin = Map.minKeyValue m"
            "    let kmax, vmax = Map.maxKeyValue m"
            "    print (string kmin + vmin + \" \" + string kmax + vmax)"
        ]
