    ...
    testList "generators (source-level plugins)" [
        ...
        test "a generator sees hand-written declarations only" {
            ...
            let emitsAType : Fpp.Core.Plugins.Generator =
                { GName = "emitsAType"
                  GAfter = None
                  Generate = fun _ -> [ "extra.fpp", Source "type Injected = { Q : int }\n" ] }
            let reportsWhatItSaw : Fpp.Core.Plugins.Generator =
                { GName = "reportsWhatItSaw"
                  ...
                  Generate =
                    fun view ->
                        let names = view.Types |> List.map (fun t -> t.TName) |> List.sort
                        [ "report.fpp", Source ("let seenByGenerator = \"" + String.concat "," names + "\"\n") ] }
            let out =
                        ...
                        let decls =
                            ...
                            |> List.map (fun t ->
                                ...
                                GQuote (
                                    ...
                                    + exText 4 sumEx))
                        [ "summer.fpp", Source (renderFile (opens @ decls)) ] }
            let out =
                runGenerated [ summer ]
        ...
        test "a generator's mistakes are reported against the generator" {
            ...
            let sloppy : Fpp.Core.Plugins.Generator =
                { GName = "sloppy"
                  GAfter = None
                  Generate = fun _ -> [ "sloppy.fpp", Source "let ok (n : int) = n\nlet broken (x : int) = x + * 3\n" ] }
            let ws = Workspace()
                ...
                { GName = "provider"
                  ...
                  Generate =
                    fun _ ->
                        ...
                        [ "provided.fpp", Source (renderFile decls) ] }
            let out =
                ...
                { GName = "accessors"
                  ...
                  Generate =
                    fun view ->
                        ...
                        [ "accessors.fpp", Source (renderFile (opens @ perType)) ] }
            let out =
                ...
                { GName = "tracer"
                  ...
                  Generate =
                    fun view ->
                        ...
                        [ "traced.fpp", Source (renderFile [ GValue ("tracedNames", [], None, GStr (String.concat "," traced)) ]) ] }
            let out =
                runGenerated [ tracer ]
                    [ "types.fpp",
                      [ "module Types"
        ...
        test "a rewritten file still gets its errors blamed on the plugin" {
            let breaker : Fpp.Core.Plugins.Generator =
                { GName = "breaker"
                  GAfter = None
                  Generate = fun view -> view.Sources |> List.map (fun (p, _) -> p, Source "let x = 1 +\n") }
            let ws = Workspace()
        ...
        test "a plugin gets each file's TREE and types, and can return a tree" {
            // tree in, tree out: the identity plugin hands back exactly what it
            // was given, and the program still compiles and runs
            let identity : Fpp.Core.Plugins.Generator =
                { GName = "identity"
                  GAfter = None
                  Generate = fun view -> view.Files |> List.map (fun f -> f.FPath, Tree f.FTree) }
            let out =
                runGenerated [ identity ]
                    [ "main.fpp", [ "module Main"; "let f (x : int) = x + 1"; "let go = print (f 41)" ] ]
        ...
        test "a plugin can read the type inference gave each definition" {
            let sigdump : Fpp.Core.Plugins.Generator =
                { GName = "sigdump"
                  ...
                  Generate =
                    fun view ->
                        let sigOf (v : GenValueDecl) =
                            let file = view.Files |> List.tryFind (fun f -> f.FPath = v.VFile)
                            let off =
                                match file with
                                | Some f ->
                                    Fpp.Syntax.Green.tokens (Fpp.Syntax.GNode f.FTree)
                                    |> List.tryPick (fun t ->
                                        if t.Kind = Fpp.Syntax.Ident && t.Text = v.VName then Some t.Offset else None)
                                | None -> None
                            let ty =
                                match file, off with
                                | Some f, Some o -> f.FTypeAt o
                                | _ -> None
                            v.VName + " : " + (match ty with Some t -> t | None -> "?")
                        [ "sigs.fpp",
                          Source ("let sigs = \"" + String.concat "; " (List.map sigOf view.Values) + "\"\n") ] }
            let out =
                runGenerated [ sigdump ]
                    [ "types.fpp", [ "module Types"; "let twice (x : int) = x * 2"; "let name = \"n\"" ]
                      "main.fpp", [ "module Main"; "let go = print sigs" ] ]
            Expect.equal out "twice : int -> int; name : string; go : 'a\n"
                "the inferred type of every definition is available to a plugin"
        }
