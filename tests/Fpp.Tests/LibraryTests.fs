...
let private oracleFile (relPath : string) (stripPrefix : string) =
    ...
    let actual = run (home + "/.wasmtime/bin/wasmtime") ("run -W gc=y,exceptions=y --preload fpprt=" + Fpp.Tests.WasmRun.reactor + " " + tmp)
...
let acceptanceTests =
    testList "acceptance: HashCollections" [
        test "reference HashCollections source runs HashSet and HashMap" {
            ...
            let psi =
                let home = System.Environment.GetFolderPath System.Environment.SpecialFolder.UserProfile
                System.Diagnostics.ProcessStartInfo(home + "/.wasmtime/bin/wasmtime", "run -W gc=y,exceptions=y --preload fpprt=" + Fpp.Tests.WasmRun.reactor + " " + tmp)
            psi.RedirectStandardOutput <- true
            psi.RedirectStandardError <- true
            let actual, err, code =
                use p = System.Diagnostics.Process.Start psi
