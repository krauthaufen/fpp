/* avl, with REAL memory management: malloc per node and REFCOUNTS, because
 * the tree is PERSISTENT — each insert shares most of its nodes with the
 * version before it, so "free the old tree" would free nodes the new one
 * still points at. Refcounting is what a C programmer actually has to write
 * here, and the inc/dec per edge is what it costs.
 *
 * The arena twin beside this one never frees at all. That is the honest
 * comparison to a collector: this one. */
#include <stdio.h>
#include <stdlib.h>
