...
## Structural comparison of a record follows DECLARATION order

The runtime comparator walks an object's payload words, and the inline-value
layout lays a record out in the order its fields are DECLARED — a scalar at a
multiple of its own width, a reference field one word, the size rounded up to
the widest member so an array of the record strides the way C says. A record
whose references are not a contiguous suffix carries an explicit ref-offset
map (fpprt's `FK_STRUCT`) instead of the "refs from word N on" form.

So `compare` on a record agrees with F#, and with a hand-written
`instance Ordered<T>`, whatever order the fields are written in. `Ordered` is
