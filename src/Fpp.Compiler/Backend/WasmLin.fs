...
type private St =
    { M : Mod
      ...
      /// per slot-witness param, its INDEX in the callee's quantified order —
      /// how a DIRECT call picks it out of `EVarI.inst`, which names the whole
      /// scheme. -1 is a pad the impl does not use.
      SlotWitIdx : Dict<string, int list>
...
/// A visited-set bucket for a Core expression: cheap and STABLE (it reads
/// only the constructor shape, never a mutable field), which is all an
/// identity-keyed map needs.
let private exprBucket (e : Expr) : int =
    match e with
    | EIndex (k, _, _) -> hash k * 8 + 1
    | EIndexSet (k, _, _, _) -> hash k * 8 + 2
    | EVar _ -> 3
    | ELit _ -> 4
    | _ -> 5

/// ---- eliding the bounds check ------------------------------------------
/// An access is safe when the index is known BOTH >= 0 and < the array's
/// length, and the two halves are tracked separately: a counter that only
/// decrements keeps its upper bound while losing its lower one, and one that
/// only increments does the reverse. Keeping them apart is what lets a
/// quicksort partition prove anything at all.
///
///   Lo : var -> a known lower bound (>= 0 to be useful)
///   Hi : "array|var" -> that var is below THAT array's length
///
/// Facts come from four places: a counted loop's own guard, a comparison in
/// an enclosing `if`/`while`, a check that already ran, and — across function
/// boundaries — a PRECONDITION on a parameter (see inferPreconditions).
let rec private assignsVar (k : string) (e : Expr) : bool =
    match e with
    | EAssign (v, r) -> key v = k || assignsVar k r
    | _ -> children e |> List.exists (assignsVar k)

/// every value assigned to `k` anywhere in `e`
let rec private assignedValues (k : string) (e : Expr) : Expr list =
    match e with
    | EAssign (v, r) when key v = k -> r :: assignedValues k r
    | _ -> children e |> List.collect (assignedValues k)

let private intLit (e : Expr) : int option =
    match e with
    | ELit (LInt n) when n.Length > 0 ->
        let neg = n.[0] = '-'
        let ds = if neg then n.Substring 1 else n
        let mutable v = 0
        let mutable ok = ds.Length > 0
        for c in ds do
            if c >= '0' && c <= '9' then v <- v * 10 + (int c - int '0') else ok <- false
        if ok then Some (if neg then 0 - v else v) else None
    | _ -> None

/// `v <- v + k` for a positive k, and nothing else: the lower bound survives
let private onlyRises (k : string) (body : Expr) : bool =
    assignedValues k body
    |> List.forall (fun r ->
        match r with
        | EPrim ("+", [ EVar (v, _); d ]) -> key v = k && (match intLit d with Some n -> n >= 0 | None -> false)
        | EPrim ("-", [ EVar (v, _); d ]) -> key v = k && (match intLit d with Some n -> n <= 0 | None -> false)
        | _ -> false)

/// `v <- v - k` for a positive k, and nothing else: the upper bound survives
let private onlyFalls (k : string) (body : Expr) : bool =
    assignedValues k body
    |> List.forall (fun r ->
        match r with
        | EPrim ("-", [ EVar (v, _); d ]) -> key v = k && (match intLit d with Some n -> n >= 0 | None -> false)
...
/// `let a = Array.zeroCreate n` at module level: the array's length IS `n`,
/// for as long as neither name is ever assigned. A loop guarded by `i < n`
/// then bounds every such array, which is the shape a program writes by hand
/// — the guard mentions the LENGTH, so nothing keyed on the array sees it.
let mutable private globalArrLen : Map<string, string> = Map.empty
/// module-level `let n = <literal>`, so a length written as a name can still
/// be compared against a constant (`qsort 0 (n - 1)` needs n >= 1)
let mutable private globalIntLit : Map<string, int> = Map.empty
/// PRECONDITIONS: "funcKey|paramIndex|arrayKey" — this parameter is a valid
/// index into that array at every call
let mutable private paramInRange : Set<string> = Set.empty
/// accesses PROVEN in range, by node identity. Module-level because the
/// analysis is: a precondition on one function's parameter is established by
/// its callers, so nothing can be decided one body at a time.
let mutable private safeIdx : RefMap<Expr, bool> = refMapNew exprBucket
/// funcKey -> arity, and funcKey -> its parameter keys: the call-site view
/// the fixpoint needs while it walks callers
let mutable private callArity : Map<string, int> = Map.empty
let mutable private calleeParams : Map<string, string list> = Map.empty



/// counters for FPP_BOUNDS_STATS — wall-clock on these benchmarks moves by
/// 10% between runs, so "did the pass fire" is answered by counting, not by
/// timing
...
/// a non-empty array
let private lenAtLeastOne (lk : string) : bool =
    if lk.StartsWith "#lit:" then
        match intLit (ELit (LInt (lk.Substring 5))) with Some v -> v >= 1 | None -> false
    else match Map.tryFind lk globalIntLit with Some v -> v >= 1 | None -> false

let private buildArrLen (decls : Decl list) : Map<string, string> =
    let candidates =
        decls
        |> List.choose (fun d ->
            match d with
            | DLet (_, v, _, EArrayCreate (_, len, _)) ->
                (match lenKeyOf len with Some lk -> Some (key v, lk) | None -> None)
            | _ -> None)
    if List.isEmpty candidates then Map.empty
    else
        let bodies = decls |> List.choose (fun d -> match d with DLet (_, _, _, b) -> Some b | _ -> None)
        candidates
        |> List.filter (fun (ak, lk) ->
            not (bodies |> List.exists (fun b ->
                    assignsVar ak b || (not (lk.StartsWith "#lit:") && assignsVar lk b))))
        |> Map.ofList

/// ---- the abstract state -------------------------------------------------

type private Bounds =
    { Lo : Map<string, int>      // v >= this
      Hi : Set<string>           // "array|v" : v < that array's length
      /// a boolean binding and the comparison it holds. `for i in 0 .. n - 1`
      /// lowers to `let go = i <= hi in while go do ...`, so the loop's real
...
let private entryBoundsWith (facts : Set<string>) (pks : string list) : Bounds =
    pks |> List.fold (fun acc pk ->
        Map.toList globalArrLen
        |> List.fold (fun a2 (ak, _) ->
               if Set.contains (ak + "|" + pk) facts
               then { a2 with Lo = Map.add pk 0 a2.Lo; Hi = Set.add (ak + "|" + pk) a2.Hi }
               else a2) acc) emptyB

let private inRange (b : Bounds) (ak : string) (vk : string) : bool =
    (match Map.tryFind vk b.Lo with Some n -> n >= 0 | None -> false)
    && Set.contains (pairKey ak vk) b.Hi

/// forget everything about a variable
let private forget (b : Bounds) (vk : string) : Bounds =
...
let private forgetAssigned (b : Bounds) (e : Expr) : Bounds =
    let ks =
        let rec go (x : Expr) : string list =
            match x with
            | EAssign (v, r) -> key v :: go r
            | _ -> children x |> List.collect go
        go e
    ks |> List.fold forget b

/// What an index EXPRESSION is worth, given the state. Returns whether it is
/// provably in range for `ak`.
let rec private idxInRange (b : Bounds) (ak : string) (ix : Expr) : bool =
    match ix with
    | EVar (v, _) -> inRange b ak (key v)
    // a non-negative literal below a KNOWN length
    | ELit (LInt _) ->
        (match intLit ix, Map.tryFind ak globalArrLen with
         | Some v, Some lk when v >= 0 ->
             if lk.StartsWith "#lit:" then
                 (match intLit (ELit (LInt (lk.Substring 5))) with Some n -> v < n | None -> false)
             else (match Map.tryFind lk globalIntLit with Some n -> v < n | None -> false)
         | _ -> false)
    // A subtraction reads two ways and BOTH must be tried — writing them as
    // separate arms let `n - 1` match the first and never reach the second,
    // which killed the base case of every recursion bounded by `n - 1`.
    //
    //   `i - k`   the upper bound carries down; the lower one needs i >= k
    //   `len - 1` the last index of an array KNOWN to be non-empty
    | EPrim ("-", [ x; d ]) ->
        let asOffset =
            match x, intLit d with
            | EVar (v, _), Some n when n >= 0 ->
                Set.contains (pairKey ak (key v)) b.Hi
                && (match Map.tryFind (key v) b.Lo with Some lo -> lo >= n | None -> false)
            | _ -> false
        let asLastIndex =
            match lenKeyOf x, intLit d, Map.tryFind ak globalArrLen with
            | Some lk, Some 1, Some alk when lk = alk -> lenAtLeastOne lk
            | _ ->
                (match lenKeyOf x, intLit d, Map.tryFind ak b.ArrLen with
                 | Some lk, Some 1, Some alk when lk = alk -> lenAtLeastOne lk
                 | _ -> false)
    ...
    | EPrim ("/", [ len; d ]) when
          (match intLit d with Some k -> k >= 2 | None -> false)
          && (match lenKeyOf len, Map.tryFind ak globalArrLen with
              | Some lk, Some alk -> lk = alk && lenAtLeastOne lk
              | _ ->
                  (match lenKeyOf len, Map.tryFind ak b.ArrLen with
                   | Some lk, Some alk -> lk = alk && lenAtLeastOne lk
                   | _ -> false)) -> true
    // the MIDPOINT of two in-range values is between them, so it is in range
    // too — whichever way round they are, since `lo + (hi - lo) / 2` lands in
    // [hi, lo] when hi < lo. No ordering fact needed.
    | EPrim ("+", [ x; EPrim ("/", [ EPrim ("-", [ y; x2 ]); d ]) ]) when d = ELit (LInt "2") ->
        idxInRange b ak x && idxInRange b ak y && exprEq x x2
    | EPrim ("/", [ EPrim ("+", [ x; y ]); d ]) when d = ELit (LInt "2") ->
        idxInRange b ak x && idxInRange b ak y
    | _ -> false

/// structural equality on the small expression shapes this analysis compares
and private exprEq (a : Expr) (b : Expr) : bool =
    match a, b with
    | EVar (x, _), EVar (y, _) -> key x = key y
    | ELit (LInt x), ELit (LInt y) -> x = y
    | _ -> false
...
let rec private applyGuard (b : Bounds) (cond : Expr) : Bounds =
    ...
    // and — if y happens to BE some array's length — x is under that array.
    // Writing them as separate match arms let the first shadow the last, and
    // `while i < n` over `Array.zeroCreate n` proved nothing.
    | EPrim (("<" | "<="), [ EVar (x, _); rhs ]) ->
        let strict = (match cond with EPrim ("<", _) -> true | _ -> false)
        let xk = key x
        let b1 =
            match rhs with
            | EVar (y, _) ->
                ...
                let hi2 =
                    b.Hi
                    |> Set.filter (fun p -> p.EndsWith ("|" + yk))
                    |> Set.map (fun p -> p.Substring (0, p.Length - yk.Length - 1))
                    |> Set.fold (fun acc ak -> Set.add (pairKey ak xk) acc) b.Hi
                let lo2 =
                    match Map.tryFind xk b.Lo with
                    | Some n -> Map.add yk (if strict then n + 1 else n) b.Lo
                    | None -> b.Lo
        ...
        if not strict then b1
            ...
            match rhs with
            | EArrayLen (_, EVar (a, _)) -> { b1 with Hi = Set.add (pairKey (key a) xk) b1.Hi }
            | _ ->
                (match lenKeyOf rhs with
                 | Some bk ->
                     (Map.toList globalArrLen @ Map.toList b1.ArrLen)
                     |> List.filter (fun (_, lk) -> lk = bk)
                     |> List.fold (fun acc (ak, _) -> { acc with Hi = Set.add (pairKey ak xk) acc.Hi }) b1
                 | None -> b1)
    // `x >= k` / `x > k` against a literal: the lower half
    | EPrim (">=", [ EVar (x, _); d ]) ->
        (match intLit d with Some n -> { b with Lo = Map.add (key x) n b.Lo } | None -> b)
    | EPrim (">", [ EVar (x, _); d ]) ->
...
/// by the precondition fixpoint.
let mutable private callObs : (string * int * string * bool) list = []
let mutable private observing = false
/// call shapes the fixpoint cannot reason about (partial application, a
/// function used as a value): every precondition on that callee dies
let mutable private opaqueCallees : Set<string> = Set.empty

let rec private provenWalk
        (safe : RefMap<Expr, bool>) (arity : Map<string, int>)
        (b : Bounds) (e : Expr) : Bounds =
    let walk st x = provenWalk safe arity st x
    match e with
    | EIndex (_, EVar (a, _), ix) ->
        let b1 = walk b ix
        if idxInRange b1 (key a) ix then refMapSet safe e true
        // whether or not it was proven, AFTER the access the index is known
        // in range — a check ran, or one was already known
        (match ix with
         | EVar (v, _) ->
             { b1 with
                 Lo = (match Map.tryFind (key v) b1.Lo with Some n when n >= 0 -> b1.Lo | _ -> Map.add (key v) 0 b1.Lo)
                 Hi = Set.add (pairKey (key a) (key v)) b1.Hi }
         | _ -> b1)
    | EIndexSet (_, EVar (a, _), ix, v) ->
        let b1 = walk b v
        let b1 = forgetAssigned b1 v
        let b2 = walk b1 ix
        if idxInRange b2 (key a) ix then refMapSet safe e true
        (match ix with
         | EVar (x, _) ->
             { b2 with
                 Lo = (match Map.tryFind (key x) b2.Lo with Some n when n >= 0 -> b2.Lo | _ -> Map.add (key x) 0 b2.Lo)
                 Hi = Set.add (pairKey (key a) (key x)) b2.Hi }
         | _ -> b2)
    | EAssign (v, rhs) ->
        let b1 = walk b rhs
        let vk = key v
        let cleared = forget b1 vk
        // `v <- v + k` keeps the lower bound, `v <- v - k` the upper one
        (match rhs with
         | EPrim ("+", [ EVar (s, _); d ]) when key s = vk ->
             (match intLit d, Map.tryFind vk b1.Lo with
              | Some n, Some lo when n >= 0 -> { cleared with Lo = Map.add vk lo cleared.Lo }
              | _ -> cleared)
         | EPrim ("-", [ EVar (s, _); d ]) when key s = vk ->
             (match intLit d with
              | Some n when n >= 0 ->
                  let kept = b1.Hi |> Set.filter (fun p -> p.EndsWith ("|" + vk))
                  { cleared with Hi = Set.union cleared.Hi kept }
              | _ -> cleared)
         // assigning an expression that is itself in range for some array
         | _ ->
             globalArrLen
             |> Map.toList
             |> List.fold (fun acc (ak, _) ->
                    if idxInRange b1 ak rhs then
                        { acc with Lo = Map.add vk 0 acc.Lo; Hi = Set.add (pairKey ak vk) acc.Hi }
                    else acc) cleared)
    | ELet (_, v, _, rhs, cont) ->
        let b1 = walk b rhs
        let vk = key v
        let b2 = forget b1 vk
        // a binding inherits whatever its right-hand side was worth
        let b3 =
            match intLit rhs with
            | Some n -> { b2 with Lo = Map.add vk n b2.Lo }
            | None ->
                globalArrLen
                |> Map.toList
                |> List.fold (fun acc (ak, _) ->
        ...
        // `hi = a.Length - 1` is BELOW a's length whatever the length is —
        // an empty array gives -1, which is still less than 0
        let b4 =
            match rhs with
            | EPrim ("-", [ EArrayLen (_, EVar (a, _)); d ]) when (match intLit d with Some n -> n >= 1 | None -> false) ->
                { b3 with Hi = Set.add (pairKey (key a) vk) b3.Hi }
            | EPrim ("-", [ len; d ]) when (match intLit d with Some n -> n >= 1 | None -> false) ->
                (match lenKeyOf len with
                 | Some lk ->
                     (Map.toList globalArrLen @ Map.toList b3.ArrLen)
                     |> List.filter (fun (_, l2) -> l2 = lk)
                     |> List.fold (fun acc (ak, _) -> { acc with Hi = Set.add (pairKey ak vk) acc.Hi }) b3
                 | None -> b3)
            // a boolean binding holding a comparison: remember WHICH one
            | EPrim (("<" | "<=" | ">" | ">="), _) -> { b3 with Flag = Map.add vk rhs b3.Flag }
            // a LOCAL array and the length it was made with
            | EArrayCreate (_, len, _) ->
                (match lenKeyOf len with
                 | Some lk -> { b3 with ArrLen = Map.add vk lk b3.ArrLen }
                 | None -> b3)
            | _ -> b3
        walk b4 cont
    | EIf (c, t, f) ->
        let b1 = walk b c
        walk (applyGuard b1 c) t |> ignore
        walk b1 f |> ignore
        b1
    | EMatch (scrut, cs) ->
        let b1 = walk b scrut
        for (_, g, body) in cs do
            let b2 = match g with Some x -> applyGuard (walk b1 x) x | None -> b1
            walk b2 body |> ignore
        b1
    | ETry (body, cs) ->
        walk b body |> ignore
        for (_, g, h) in cs do
            (match g with Some x -> walk b x |> ignore | None -> ())
            walk b h |> ignore
        // an exception can leave the body part-way through
        emptyB
    | ELam (_, body) ->
        provenWalk safe arity emptyB body |> ignore
        b
    | EWhile (cond, body) ->
        // The guard is re-evaluated after every iteration, so an access
        // INSIDE it may only use what survives the body's writes — not the
        // richer state that held before the loop. Walking it with the
        // pre-loop state proved `while a.[i] < p do i <- i + 1`, which is
        // exactly the scan nothing can prove: i's upper bound is gone the
        // moment it is incremented.
        let b1 = walk (loopCarried b body) cond
        // the guard holds on entry to the body; the body re-enters, so it
        // starts from the guard alone plus whatever survives its own writes
        // `while go do` where `go` holds `i <= hi`: the comparison is the real
        // guard. Sound while the body only ever CLEARS the flag — the loop
        // runs when it was last true, and the counter's step preserves it.
        let realCond =
            match cond with
            | EVar (g, _) when
                  (assignedValues (key g) body
                   |> List.forall (fun r -> match r with ELit (LBool false) -> true | _ -> false)) ->
                (match Map.tryFind (key g) b1.Flag with Some c -> c | None -> cond)
            | _ -> cond
        let entry = applyGuard b1 realCond
        provenWalk safe arity entry body |> ignore
        // AFTER the loop, a counter that only fell keeps its upper bound and
        // one that only rose keeps its lower one — the same reasoning that
        // carries them ACROSS iterations. Forgetting both is what left
    ...
    | EApp (EVar (fv, _), args) | EApp (EVarI (fv, _, _), args) ->
        ...
        if observing then
            let fk = key fv
            (match Map.tryFind fk arity, Map.tryFind fk calleeParams with
             | Some n, Some pks when n = List.length args ->
                 List.iteri2 (fun _ (pk : string) (a : Expr) ->
                     for (ak, _) in Map.toList globalArrLen do
                         let ok = idxInRange b1 ak a
                         if not ok && System.Environment.GetEnvironmentVariable "FPP_BOUNDS_DUMP" = "1" then
                             eprintfn "NOSUPPORT %s arg=%s" pk (printExpr a)
                         callObs <- (pk, 0, ak, ok) :: callObs) pks args
             | _ -> opaqueCallees <- Set.add fk opaqueCallees)
        b1
    | EVar (fv, _) | EVarI (fv, _, _) ->
        // a function named but not applied: nothing can be assumed of its
        // parameters any more
        if observing && (Map.tryFind (key fv) arity).IsSome then
            opaqueCallees <- Set.add (key fv) opaqueCallees
        b
    | _ -> children e |> List.fold walk b

/// What survives into the NEXT iteration of a loop: a bound the body cannot
/// disturb, plus the one-sided bounds a monotone counter keeps.
and private loopCarried (b : Bounds) (body : Expr) : Bounds =
    let assigned =
        let rec go (x : Expr) : string list =
            match x with
            | EAssign (v, r) -> key v :: go r
        ...
        go body |> Set.ofList
    let lo =
        b.Lo |> Map.filter (fun k _ -> not (Set.contains k assigned) || onlyRises k body)
    let hi =
        b.Hi |> Set.filter (fun p ->
            match p.Split '|' with
...
/// body — its CALLERS establish it. Start by assuming every such fact, then
/// drop the ones some call site fails to support, and repeat. The assumption
/// is discharged by induction on the execution trace: a call is supported by
/// its caller's facts, which held when the caller's own activation began.
///
/// This is what makes a recursive partition provable. In quicksort,
/// `qsort lo j` has j <= hi because j only falls from hi, `qsort i hi` has
/// i >= lo because i only rises from lo, and `qsort 0 (n - 1)` is the base —
/// so `lo` and `hi` are in range at every call, and `a.[lo + (hi - lo) / 2]`
/// follows from that.
let private inferPreconditions (safe : RefMap<Expr, bool>) (decls : Decl list) : unit =
    let funcs =
        decls |> List.choose (fun d ->
            match d with
            | DLet (_, v, _, ELam (ps, body)) -> Some (key v, List.length ps, ps, body)
            | _ -> None)
    let arity = funcs |> List.map (fun (k, n, _, _) -> (k, n)) |> Map.ofList
    // funcKey -> its parameters' keys, so a fact can name the PARAMETER
    // rather than its position
    let paramsOf =
        funcs |> List.map (fun (k, _, ps, _) -> (k, ps |> List.map (fun (pv, _) -> key pv))) |> Map.ofList
    calleeParams <- paramsOf
    // The FIXPOINT needs functions and arrays to say anything; the MARKING
    // pass below runs either way, since a loop proves its own accesses with
    // no preconditions at all. Skipping both when there were no functions
    // turned every elision off in modules that are just top-level loops.
    // optimistic start: every (function, parameter, array) triple
    let mutable facts =
        funcs
        |> List.collect (fun (_, _, ps, _) ->
            // written with collect rather than a nested comprehension: this
            // file is compiler SOURCE, and the self-hosted compiler does not
            // lower `[ for x in xs do for y in ys -> ... ]`
            ps |> List.collect (fun (pv, _) ->
                Map.toList globalArrLen |> List.map (fun (ak, _) -> pairKey ak (key pv))))
        |> Set.ofList
    let bodies = decls |> List.choose (fun d -> match d with DLet (_, _, _, b) -> Some b | _ -> None)
    let mutable changed = not (Map.isEmpty globalArrLen) && not (List.isEmpty funcs)
    ...
    while changed && rounds < 8 do
        ...
        paramInRange <- facts
        callObs <- []
        opaqueCallees <- Set.empty
        observing <- true
        // walk every body under the CURRENT assumption, recording what each
        // call site could prove about its arguments
        for (fk, _, ps, body) in funcs do
            ignore fk
            provenWalk safe arity (entryBoundsWith facts (ps |> List.map (fun (pv, _) -> key pv))) body |> ignore
        // the top-level statements are callers too, with no parameters
        for d in decls do
            match d with
            | DLet (_, _, _, ELam _) -> ()
            | DLet (_, _, _, b) -> provenWalk safe arity emptyB b |> ignore
            | _ -> ()
        observing <- false
        // a fact survives only if EVERY observed call supported it
        let unsupported =
            callObs
            |> List.filter (fun (_, _, _, ok) -> not ok)
            |> List.map (fun (pk, _, ak, _) -> pairKey ak pk)
            |> Set.ofList
        // a callee named but not called through a known application loses
        // every precondition: nothing constrains what reaches its parameters
        let opaqueParams =
            opaqueCallees
            |> Set.toList
            |> List.collect (fun fk -> match Map.tryFind fk paramsOf with Some ps -> ps | None -> [])
            |> Set.ofList
        let dead =
            facts |> Set.filter (fun f ->
                Set.contains f unsupported
                || (match f.Split '|' with
                    | [| _; pk |] -> Set.contains pk opaqueParams
                    | _ -> true))
        if not (Set.isEmpty dead) then
            facts <- Set.difference facts dead
            changed <- true
    // The facts are the OUTPUT. Marking happens later, per function, against
    // the body emission is about to lower: the bodies walked here are the
    // pre-transform ones, and node identity does not survive lifting and
    // stamping. Facts key on the PARAMETER, which does.
    paramInRange <- facts
    callArity <- arity
    if System.Environment.GetEnvironmentVariable "FPP_BOUNDS_STATS" = "1" then
        eprintfn "PRECOND: %d facts over %d funcs, %d arrays" (Set.count facts) (List.length funcs) (Map.count globalArrLen)
    ignore bodies
...
let private hiddenWitCount (st : St) (k : string) : int =
    match dictTryFind st.FuncWitness k with
    | Some ws -> List.length ws
    | None -> (match dictTryFind st.SlotWitness k with Some ws -> List.length ws | None -> 0)
...
/// The witness for one type-arg NAME, or None when the name is a VARIABLE this
/// body holds none for. The caller decides what to do about it — a direct call
/// still has the callee's own variable id to try (a recursive or group call
/// shares it), and only when that fails too is the answer uniform.
...
and private coreToLowEBody (ctx : LowCtx) (e : Expr) : LExpr =
    ...
    | EField ((EIndex (ek, arr, i)) as idxNode, fname, _) when
        ...
        // `a.[i].X` fuses the element read into the FIELD load, so it never
        // reaches the EIndex case — it needs the same guard, keyed on the
        // inner access node. A guarded read has to bind the base anyway, so
        // the inline fast paths apply only when there is nothing to check.
        let needsGuard = not (List.isEmpty (boundsGuardPeek ctx idxNode))
        if not needsGuard && lowSafeE arrE && lowSafeE iE && lowSafeE direct then direct
        ...
        // reach.
        elif not needsGuard && lowSafeE arrE && lowSafeE iE then
    ...
    | EIndex (ek, arr, i) when (podArrOf st ek).IsSome ->
        ...
        LDo ([ LSet (wReg ar, coreToLowE ctx arr); LSet (wReg ir, (coreToLowE ctx i)) ]
             @ boundsGuard ctx e ar ir, lowPodBuild ctx ek items)
    ...
    | EIndexSet (ek, arr, i, v) when (podArrOf st ek).IsSome ->
        ...
        LDo ([ LSet (wReg vr, coreToLowE ctx v); LSet (wReg ar, coreToLowE ctx arr); LSet (wReg ir, (coreToLowE ctx i)) ]
             @ boundsGuard ctx e ar ir @ copies, lowInt 0)
    ...
    | EIndex (k, arr, i) when (storLTy (storKindRes ctx.LSt k)).IsSome ->
        ...
        let ir = freshTmp ctx
        let fv = freshTmpT ctx vty
        // an ELIDED access emits exactly what it did before the checks
        // existed — binding the base into a register defeats the hoist that
        // lifts it out of a loop, and that cost shows up whether or not a
        // check is there to pay for it
        if List.isEmpty (boundsGuardPeek ctx e) then
        ...
        LDo ([ LSet (wReg ar, coreToLowE ctx arr)
               LSet (wReg ir, (coreToLowE ctx i)) ]
             @ boundsGuard ctx e ar ir
             @ [ LSet ({ Id = fv; RTy = vty }, LLoad (sty, LPrim (AddW, [ LGet (wReg ar); LPrim (MulW, [ LGet (wReg ir); LConstW w ]) ]), HDR + 4)) ],
             storBox ctx k (LGet { Id = fv; RTy = vty }))
    | EIndexSet (k, arr, i, v) when (storLTy (storKindRes ctx.LSt k)).IsSome ->
        ...
        LDo ([ LSet ({ Id = fv; RTy = vty }, storUnbox k (coreToLowE ctx v))
               ...
               LSet (wReg ir, (coreToLowE ctx i)) ]
             @ boundsGuard ctx e ar ir
             @ [ LStore (sty, LPrim (AddW, [ LGet (wReg ar); LPrim (MulW, [ LGet (wReg ir); LConstW w ]) ]), HDR + 4, LGet { Id = fv; RTy = vty }) ],
             lowInt 0)
    ...
    | EIndex ("$str", arr, i) | EIndex (_, arr, i) when
            (match e with EIndex ("$str", _, _) -> true | _ -> shapeOfExpr arr = ShStr) ->
        let ir = freshTmp ctx
        if List.isEmpty (boundsGuardPeek ctx e) then
            LDo ([ LSet (wReg ir, (coreToLowE ctx i)) ],
        ...
        LDo ([ LSet (wReg ar, coreToLowE ctx arr)
               LSet (wReg ir, (coreToLowE ctx i)) ]
             @ boundsGuard ctx e ar ir,
             (LLoad (I16, LPrim (AddW, [ LGet (wReg ar); LPrim (MulW, [ LGet (wReg ir); LConstW 2 ]) ]), HDR + 4)))
    | EIndex (_, arr, i) ->
        if List.isEmpty (boundsGuardPeek ctx e) then
            LLoad (W, LPrim (AddW, [ coreToLowE ctx arr; LPrim (MulW, [ LPrim (AddW, [ (coreToLowE ctx i); LConstW 1 ]); LConstW 4 ]) ]), HDR)
        else
        let ar = freshTmp ctx
        let ir = freshTmp ctx
        let addr = LPrim (AddW, [ LGet (wReg ar); LPrim (MulW, [ LPrim (AddW, [ LGet (wReg ir); LConstW 1 ]); LConstW 4 ]) ])
        LDo ([ LSet (wReg ar, coreToLowE ctx arr); LSet (wReg ir, coreToLowE ctx i) ]
             @ boundsGuard ctx e ar ir,
             LLoad (W, addr, HDR))
    | EIndexSet (_, arr, i, v) ->
        ...
        if gc then
            ...
            LDo ([ LCallVoidS ("$spush", [ coreToLowE ctx arr ])
                   ...
                   LSet (wReg ra, LCall ("$spop", [])) ]
                 // the guard reads the length through the RE-POPPED base: a
                 // collection during the value's evaluation may have moved it
                 @ boundsGuard ctx e ra ir
        ...
        let ar = freshTmp ctx
        let ir2 = freshTmp ctx
        let addr = LPrim (AddW, [ LGet (wReg ar); LPrim (MulW, [ LPrim (AddW, [ LGet (wReg ir2); LConstW 1 ]); LConstW 4 ]) ])
        LDo ([ LSet (wReg ar, coreToLowE ctx arr); LSet (wReg ir2, coreToLowE ctx i) ]
             @ boundsGuard ctx e ar ir2
             @ [ LStore (W, addr, HDR, coreToLowE ctx v) ], lowInt 0)
    ...
    | EApp (((EVar (v, vsch) | EVarI (v, vsch, _)) as hd), args0)
        ...
        // LAST RESORT, and it closes the largest gap left: derive the witness
        // from the CALL'S ARGUMENTS. A parameter whose declared type IS the
        // variable pins it, so the argument's own witness answers for it —
        // which is exactly a constructor, `ChangeableValue<'a>(x : 'a)`, and
        // constructors are where the instantiation is missing most often
        // (inference records one at a member site, not at every `new`).
        let witFromArgs (vid : int) (pid : int) : LExpr option =
            ...
            let ptys, argEs =
                ...
                | [ p0 ], [ ETuple xs ] ->
                    (match prune p0 with
                     | TTuple ts when List.length ts = List.length xs -> ts, xs
                     | _ -> ptys0, args)
                | _ -> ptys0, args
            let n = min (List.length ptys) (List.length argEs)
            let isTheVar (pv : Var) = pv.Id = vid || pv.Id = pid || prunedId pv = vid || prunedId pv = pid
            // the argument's STATIC type, where Core still carries one
            let rec argTy (e : Expr) : Type option =
                match e with
            ...
            // no unification, so a shape that does not line up simply answers
            // nothing rather than guessing.
            let rec locate (pt : Type) (at : Type) : Type option =
                match prune pt, prune at with
                | TVar pv, a when isTheVar pv -> Some a
                | TCon (n1, ps), TCon (n2, xs) when n1 = n2 && List.length ps = List.length xs ->
                    List.zip ps xs |> List.tryPick (fun (p2, x2) -> locate p2 x2)
                | TFun (p1, p2), TFun (x1, x2) ->
                    (match locate p1 x1 with Some r -> Some r | None -> locate p2 x2)
                | TTuple ps, TTuple xs when List.length ps = List.length xs ->
                    List.zip ps xs |> List.tryPick (fun (p2, x2) -> locate p2 x2)
                | _ -> None
            let ofFound (t : Type) : LExpr option =
                match prune t with
                | TVar tv -> (match dictTryFind ctx.Witness tv.Id with Some r -> Some (LGet (wReg r)) | None -> None)
                | TCon (n, _) ->
        ...
        let witnessArgs =
            ...
            | Some vids ->
                vids |> List.mapi (fun i (vid, pid) ->
                    match (match List.tryItem i inst with
                           | Some nm when nm <> "" -> witnessArgOfNameOpt ctx nm
                           | _ -> None) with
                    | Some w -> w
                    | None ->
                        ...
                        // This is also the fallback when the call DOES name the
                        // argument but names a variable this body holds none
                        // for — the id still matches on a self/group call.
                        match dictTryFind ctx.Witness vid with
                        | Some reg -> LGet (wReg reg)
                        | None ->
                            (match dictTryFind ctx.Witness pid with
                             | Some reg -> LGet (wReg reg)
                             | None ->
            ...
            | None ->
                ...
                | Some vids ->
                    let idxs = match dictTryFind st.SlotWitIdx (key v) with Some xs -> xs | None -> []
                    vids |> List.mapi (fun i (vid, pid) ->
                        match (match List.tryItem i idxs with
                               | Some j when j >= 0 ->
                                   (match List.tryItem j inst with
                                    | Some nm when nm <> "" -> witnessArgOfNameOpt ctx nm
                                    | _ -> None)
                               | _ -> None) with
                        | Some w -> w
                        | None ->
                            ...
                            | None ->
                                ...
                                | None ->
                                    match witFromArgs vid pid with
                                    | Some w -> w
                                    | None -> uniformWitnessWhy st (if List.isEmpty inst then "slot-unnamed" else "slot-freevar"))
                | None -> []
        ...
        let slotWits =
            ...
            if k = 0 then []
                ...
                List.init k (fun i ->
                    ...
                    | _ -> uniformWitnessWhy st "slotwit-dispatch")
...
/// Checks are ON. `FPP_NO_BOUNDS=1` removes them, which exists so the cost
/// can be MEASURED against the same binary — not as a shipping mode.
/// counters for FPP_BOUNDS_STATS — wall-clock on these benchmarks moves by
/// 10% between runs, so "did the pass fire" is answered by counting, not by
/// timing
...
/// the length load and the element load share one evaluation.
...
let private emitFuncLow (st : St) (m : Mod) (dbgName : string) (brPays : Dict<string, string>) (isInit : bool) (sig_ : (LTy list * LTy) option) (structPlan : string option list) (byrefPlan : bool list) (retStruct : string option) (witnessVars : (int * int) list) (selfWits : (int * int) list) (constWits : (int * LExpr) list) (ps : VarId list) (paramTypes : Type list) (body : Expr) (finish : Fn -> unit) : unit =
    ...
    // Mark the accesses this body can prove, against the body emission will
    // actually lower — node identity does not survive the transforms between
    // the precondition fixpoint's view and this one.
    if boundsOn && System.Environment.GetEnvironmentVariable "FPP_NO_BOUNDS_ELIDE" <> "1" then
        let pks = ps |> List.map key
        provenWalk safeIdx callArity (entryBoundsWith paramInRange pks) body |> ignore
...
let private emitLinearImpl (decls1 : Decl list) : byte[] * string list =
    ...
    globalIntLit <-
        if not boundsOn then Map.empty
            ...
            |> List.choose (fun d ->
                match d with
                | DLet (_, v, _, rhs) -> (match intLit rhs with Some n -> Some (key v, n) | None -> None)
                | _ -> None)
            |> Map.ofList
    refMapClear safeIdx
    if boundsOn && System.Environment.GetEnvironmentVariable "FPP_NO_BOUNDS_ELIDE" <> "1" then
        inferPreconditions safeIdx decls0
    ...
    for d in decls do
        ...
        | _ -> ()
    // A class NOTHING can dispatch to: no base, no subclass, no interface. Its
    // rows can never be entered — an EIfaceCall needs an interface it does not
    // implement, and abstract-through-class dispatch needs a chain it does not
    // have — so they stay 0 and its members keep their specialized signature.
    // The vtable FILL and the slot-witness ABI must agree with the vtImpls
    // marking here: a row filled for a class the marking skipped would be
    // entered at an arity its impl never declared.
    let dispatchableClass (cn : string) : bool =
    ...
    // no hidden witness params, or the call_indirect type mismatches. Their rows
    // are filled by NAME (ownMemberNamed/fillIdentity), not through the slot
    // list, so the slot ABI cannot see the constraint and has to be told: one of
    // these filling some OTHER slot at k>0 would be called at arity 2 by $eqv.
    let trioFns = dictNew<string, bool> ()
    for d in decls0 do
        match d with
        | DMembers (_, own) ->
            for mn, v in own do
                if mn = "Equals" || mn = "GetHashCode" || mn = "CompareTo" then
                    dictSet vtImpls (key v) true
                    dictSet trioFns (key v) true
        | DClass (_, _, own, _) ->
            for mn, v in own do
                if mn = "Equals" || mn = "GetHashCode" || mn = "CompareTo" then
                    dictSet vtImpls (key v) true
    ...
    for d in decls0 do
        ...
        | DClass (cn, _, _, impls) ->
            ...
            if (List.length (chainOf cn) > 1 || List.length (subclassesOf cn) > 1
                || not (List.isEmpty impls)) then
    ...
    if gc then
        ...
        let methodQuantIdx = dictNew<string, int list> ()
        for d in decls do
            match d with
            | DLet (_, v, s, ELam (ps, _)) when (dictTryFind st.Funcs (key v)).IsSome ->
                ...
                // exactly what selfWits recovers off the receiver, and nothing
                // else — every quantified variable it does NOT cover is one the
                // slot must carry, class parameter or method parameter alike.
                // An UNWITNESSED receiver (a generic UNION, a class with no
                // witness area) recovers none, so all of them ride the slot.
                let selfIds =
                    match ps with
                    | (_, s0) :: _ ->
                        (match prune s0.Body with
                         | TCon (cn, _) when (dictTryFind st.ObjWit cn).IsSome ->
                             (match dictTryFind st.ObjWit cn with Some vars -> Some vars | None -> Some [])
                         ...
                         // A STAMPED CLONE recovers nothing off self and needs
                         // nothing: its class arguments are ground and baked
                         // into the name, so everything quantified is the
                         // method's own. (A generic class implementing an
                         // interface is monomorphized into exactly these.)
                         //
                         // Any OTHER receiver that selfWits cannot read is left
                         // alone. Handing its class parameters to the slot was
                         // tried and is worse: k grows, and the tail of the
                         // instantiation a call site carries then lines up with
                         // the method's parameters, not the class', so almost
                         // every call answers uniform. 173 -> 444.
                         | TCon (cn, _) when
                                (match dictTryFind st.RecBase cn with
                                 | Some b -> cn.StartsWith (b + "$")
                                 | None -> false) -> Some []
                         | _ -> None)
                    | [] -> None
                (match selfIds with
                 | Some ids ->
                     // and it must OCCUR in the member's own type. A variable the
                     // signature never mentions is phantom: no call site can name
                     // it, so a hidden parameter for it is one every caller has to
                     // answer "uniform" to. `Inner.set_Count` carried one, and it
                     // put a fallback at each of its 30-odd call sites.
                     let recvIds =
                         match ps with
                         | (_, s0) :: _ -> Fpp.Analysis.Types.freeVars (prune s0.Body) |> List.map (fun x -> x.Id)
                         | [] -> []
                     // an abstract OVERRIDE carries the abstract signature, with
                     // no receiver in it — the receiver's own variables count as
                     // mentioned too, or a class parameter reads as phantom
                     let sigIds = recvIds @ (Fpp.Analysis.Types.freeVars (prune s.Body) |> List.map (fun x -> x.Id))
                     let own =
                         s.Quantified
                         |> List.filter (fun q ->
                             not (List.contains q.Id ids) && not (List.contains (prunedId q) ids)
                             && (List.contains q.Id sigIds || List.contains (prunedId q) sigIds))
                     ...
                     // their INDEX in the scheme's quantified order is what a
                     // DIRECT call uses to pick them out of `EVarI.inst`; a
                     // dispatch has only the tail, so they must also BE the tail
                     let idxOf (q : Var) =
                         match s.Quantified |> List.tryFindIndex (fun q2 -> q2.Id = q.Id) with
                         | Some i -> i
                         | None -> 0 - 1
                     ...
                     if n > 0 && List.length tail = n
                        && List.forall2 (fun (a2 : Var) (b2 : Var) -> a2.Id = b2.Id) own tail then
                         ...
                         dictSet methodQuantIdx (key v) (own |> List.map idxOf)
                 | None -> ())
            | _ -> ()
        ...
        // EVERY row filler, dispatchable or not — the fill loop below files them
        // all, and a slot's k must account for each one or a row is entered at an
        // arity its impl never declared
        for cn, _, _, _ in classDecls do
            if true then
                vtableSlots |> List.iteri (fun slot (ifn, mn) ->
                    ...
                    | _ -> ())
        // THE IDENTITY TRIO'S ROWS ARE FILLED BY NAME, not through the slot list
        // ($eqv/$cmpv/$hashv read slots 0/1/2 directly at arity 2). Record those
        // fillers against slots 0/1/2 — whose k is 0 — so the fixpoint below
        // drags every OTHER slot the same function fills down with them. Without
        // it a class' Equals could take a witness param for `IEquatable.Equals`
        // and be called at arity 2 through row 0: "indirect call type mismatch",
        // with the trap deep inside the structural walker.
        (let recordAt (slot : int) (v : VarId) =
            let k2 = key v
            dictSet implVar k2 v
            (match dictTryFind implSlots k2 with
             | Some ss -> if not (List.contains slot ss) then dictSet implSlots k2 (ss @ [ slot ])
         ...
         for d in decls0 do
            match d with
            | DMembers (_, own) ->
                for slot, mn in trioNames do
                    for mm, v in own do
                        if mm = mn && (dictTryFind st.Funcs (key v)).IsSome then recordAt slot v
            | DClass (cn, _, _, _) ->
                for slot, mn in trioNames do
                    for c in chainOf cn do
                        for (_, _, own, _) in declsNamed c do
                            for mm, v in own do
                                if mm = mn && (dictTryFind st.Funcs (key v)).IsSome then recordAt slot v
            | _ -> ())
        for slot, keys in dictPairs slotImplKeys do
            ...
            let k =
                if slot < 3 then 0
                    ...
                    if keys |> List.exists (fun k2 ->
                            (dictTryFind st.FuncWitness k2).IsSome || (dictTryFind trioFns k2).IsSome) then 0
                    else
        ...
        for k2, slots in dictPairs implSlots do
            ...
            if k > 0 && (dictTryFind st.FuncWitness k2).IsNone then
                ...
                let ownIdx = match dictTryFind methodQuantIdx k2 with Some xs -> xs | None -> []
                dictSet st.SlotWitIdx k2 (List.init (k - List.length ownIdx) (fun _ -> 0 - 1) @ ownIdx)
        ...
        | Some cid ->
            ...
            vtableSlots |> List.iteri (fun slot (ifn, mn) ->
                ...
                | Some v when (dictTryFind st.Funcs (key v)).IsSome ->
                    (if vtdbg then eprintfn "VT %s cid=%d slot=%d %s.%s -> %s hidden=%d k=%d" cn cid slot ifn mn v.Name (hiddenWitCount st (key v)) (match dictTryFind st.SlotWitN slot with Some x -> x | None -> 0))
...
let private withStats (r : byte[] * string list) : byte[] * string list =
    if System.Environment.GetEnvironmentVariable "FPP_BOUNDS_STATS" = "1" then
        eprintfn "BOUNDS: %d emitted, %d elided (%d%% elided)"
