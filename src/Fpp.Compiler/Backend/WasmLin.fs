...
/// A counted loop over an array PROVES its own accesses in range, and those
/// are most of them. Two shapes reach here, both generated by Lower:
///
///   for i in 0 .. a.Length - 1     let i = 0 in let hi = a.Length - 1 in
///                                  let go = i <= hi in while go do ...
///   for v in a                     let arr = a in let ix = 0 in
///                                  while ix < arr.Length do ...
///
/// In both, the counter starts at a non-negative literal, only ever moves by
/// +1, and the loop's own guard bounds it above — so `0 <= i < length` holds
/// at every body entry. The accesses that use exactly that pair are marked
/// by NODE, not by variable: marking a pair would also cover a use outside
/// the loop, where nothing was proven.
...
let private onlyIncrements (k : string) (body : Expr) : bool =
...
let rec private markPair (safe : RefMap<Expr, bool>) (ak : string) (ik : string) (e : Expr) : unit =
    (match e with
     | EIndex (_, EVar (a, _), EVar (i, _)) when key a = ak && key i = ik -> refMapSet safe e true
     | EIndexSet (_, EVar (a, _), EVar (i, _), _) when key a = ak && key i = ik -> refMapSet safe e true
     | _ -> ())
    for c in children e do markPair safe ak ik c

/// Walk a function body, marking every access already PROVEN in range.
///
/// Two sources of proof, threaded through one pass:
///
///  * a counted loop's own guard (the two shapes above), which covers the
///    accesses that use exactly the counter and the array it measured;
///  * a check that ALREADY RAN on the same pair earlier in the same
///    straight-line region — `let t = a.[i] in a.[i] <- a.[j]` checks
///    `a.[i]` once, not twice.
///
/// A fact dies when either variable is assigned. Facts do not escape a
/// branch (the join would have to intersect), do not enter a loop body (it
/// re-enters), and do not enter a lambda (it runs elsewhere).
...
let private buildArrLen (decls : Decl list) : Map<string, string> =
    ...
    if List.isEmpty candidates then Map.empty
        ...
        // an assignment to either name anywhere in the program dissolves the
        // relation; a module-level `let` is not assignable, but a shadowing
        // local of the same key would be
        ...
        |> (fun m ->
             if System.Environment.GetEnvironmentVariable "FPP_BOUNDS_STATS" = "1" then
                 eprintfn "ARRLEN: %d entries (of %d candidates)" (Map.count m) (List.length candidates)
             m)
...
let rec private provenWalk
    ...
    let killAssigned (f : Set<string>) (x : Expr) : Set<string> =
        f |> Set.filter (fun p ->
            match p.Split '|' with
    ...
    | ELet (_, v, _, rhs, cont) ->
        let f1 = walk facts rhs
        // rebinding the name invalidates anything that named it
        let f2 = f1 |> Set.filter (fun p ->
            match p.Split '|' with
            | [| a; i |] -> a <> key v && i <> key v
            | _ -> true)
        let k = key v
    ...
    | EIf (c, t, f) ->
        ...
        f1
    | EMatch (scrut, cs) ->
        let f1 = walk facts scrut
        for (_, g, b) in cs do
            let f2 = match g with Some x -> walk f1 x | None -> f1
            walk f2 b |> ignore
        f1
    | ETry (b, cs) ->
        walk facts b |> ignore
        for (_, g, h) in cs do
            (match g with Some x -> walk facts x |> ignore | None -> ())
            walk facts h |> ignore
        // an exception can leave the body part-way: keep nothing
        Set.empty
    | ELam (_, b) ->
        provenWalk safe arrLen zero hiOf goOf Set.empty b |> ignore
        facts
    | EWhile (cond, body) ->
        let loopPairs =
            match cond with
            // `while i < n` where n is the LENGTH some array was created
            // with: every such array is bounded by the same guard. This is
            // the shape a program writes by hand — `Array.zeroCreate n` at
            // the top and `while i < n` over it — and the guard mentions the
            ...
            | EPrim ("<", [ EVar (i, _); bound ]) when
                ...
                |> List.map fst
                |> List.filter (fun ak -> not (assignsVar ak body))
                |> List.map (fun ak -> (ak, key i))
            | _ -> []
        let loopPair =
            match cond with
            ...
            // `i <= hi`, and the body only clears it or bumps `i`
            | EVar (g, _) ->
                (match Map.tryFind (key g) goOf with
                 | Some (ak, ik) when
                       onlyIncrements ik body
                       && not (assignsVar ak body)
                       && (assignedValues (key g) body
                           |> List.forall (fun r -> match r with ELit (LBool false) -> true | _ -> false))
                       && (hiOf |> Map.forall (fun hk (a2, _) -> a2 <> ak || not (assignsVar hk body))) ->
                     Some (ak, ik)
                 | _ -> None)
            | _ -> None
        let f1 = walk facts cond
        // the body re-enters, so it starts from the loop's own proof alone
        let entry =
            (match loopPair with
             | Some (ak, ik) -> [ pairKey ak ik ]
             | None -> [])
            @ (loopPairs |> List.map (fun (ak, ik) -> pairKey ak ik))
            |> Set.ofList
        provenWalk safe arrLen zero hiOf goOf entry body |> ignore
        killAssigned f1 body
    | ESeq xs -> xs |> List.fold walk facts
    | _ -> children e |> List.fold walk facts
...
type private LowCtx =
    { LSt : St
      ...
      /// Element accesses PROVEN in range, by node identity. Every other one
      /// carries a bounds check; these are the ones a loop's own guard has
      /// already established (see markSafeIndices). Keyed by reference
...
and private boundsGuardPeek (ctx : LowCtx) (node : Expr) : LStmt list =
    if not boundsOn || (refMapTryFind ctx.SafeIdx node).IsSome then [] else [ LTrap ]

and private boundsGuard (ctx : LowCtx) (node : Expr) (baseR : int) (idxR : int) : LStmt list =
    if not boundsOn then []
    elif (refMapTryFind ctx.SafeIdx node).IsSome then
