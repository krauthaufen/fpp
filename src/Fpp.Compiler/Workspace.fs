...
"""
    ...
    let source =
        String.concat "\n" (numericClasses @ numericInstances @ [
            ...
            // the KeyValuePair struct the map enumerators hand out
            "[<Struct>]"
...
module private BuiltinCache =
    ...
    let value =
        lazy (
            ...
            bp, bb, binf, imports, schemes, aliases, fields, ifaces, bases, impls, structTypes, ctors, classes, members)
...
type Workspace() =
    ...
    member this.ProjectCheck () : ProjectResults =
        db.MemoT "projectCheck" "" (fun () ->
            ...
            let _, _, cbinf, cimports, cschemes, caliases, cfields, cifaces, cbases, cimpls, cstructTypes, cctors, cclasses, cmembers =
                BuiltinCache.value.Force ()
            let imports = BuiltinCache.copyDict cimports
            let schemes = BuiltinCache.copyDict cschemes
            let aliases = BuiltinCache.copyDict caliases
            let fields = BuiltinCache.copyDict cfields
            let ifaces = BuiltinCache.copyDict cifaces
            let bases = BuiltinCache.copyDict cbases
            let impls = BuiltinCache.copyDict cimpls
            let structTypes = BuiltinCache.copyDict cstructTypes
            let ctors = BuiltinCache.copyDict cctors
            // classes and instances are project-wide: the prelude declares
            // the numeric tower, every later file may extend it
            let classes = BuiltinCache.copyTables cclasses
            // members are looked up by "Type.Member" across the whole
            // project, not just the file that declares them
            let members = BuiltinCache.copyDict cmembers
    ...
    member this.EmitProgram () : string * string list =
        ...
        let bp, bb, _, _, _, _, _, _, _, _, _, _, _, _ = BuiltinCache.value.Force ()
    ...
    member this.Completions (path : string) : (string * string * string * string) list =
        ...
        let _, bb, _, _, _, _, _, _, _, _, _, _, _, _ = BuiltinCache.value.Force ()
